...
async function handleRSVPCreated(data) {
...
app.get('/api/events', async (req, res) => {
    try {
        ...
        const events = await db('events')
            ...
            .select('events.*', 'u.full_name as organizer_name', 'u.avatar as organizer_avatar', 'u.badge as organizer_badge', 'u.bio as organizer_bio', db.raw('COUNT(DISTINCT r.id) as rsvp_count'))
...
app.get('/api/events/:id', async (req, res) => {
    try {
        ...
        const event = await db('events')
            ...
            .select('events.*', 'u.full_name as organizer_name', 'u.avatar as organizer_avatar', 'u.badge as organizer_badge', 'u.bio as organizer_bio')
            .first();
        if (!event) {
...
app.post('/api/events', async (req, res) => {
    try {
        const { title, organizer_id, time, date, location, description, latitude, longitude, capacity, is_micro_apprenticeship, accessibility_notes, age_suitability, image } = req.body;
        const db = getDb();
...
app.put('/api/users/:id', async (req, res) => {
    try {
        const { full_name, age, tagline, location, avatar, career_highlights, achievements, hobbies, micro_apprenticeship_offer, offering_apprenticeship } = req.body;
        const db = getDb();
        const updateData = {
        ...
        if (career_highlights !== undefined)
            updateData.career_highlights = JSON.stringify(career_highlights);
        if (achievements !== undefined)
            updateData.achievements = JSON.stringify(achievements);
        if (hobbies !== undefined)
            updateData.hobbies = JSON.stringify(hobbies);
        if (micro_apprenticeship_offer !== undefined)
            updateData.micro_apprenticeship_offer = micro_apprenticeship_offer;
        if (offering_apprenticeship !== undefined)
            updateData.offering_apprenticeship = offering_apprenticeship;
        await db('users')
